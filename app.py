...
def create_text_frame(width=1280, height=720):
    # Chỉ tạo ảnh nền trong suốt hoặc đen
    img = Image.new("RGBA", (width, height), (0, 0, 0, 0))  # hoặc (0,0,0,255) nếu bạn muốn background đen
    return np.array(img)
...
def text_to_video(scripts, durations, output_path, fps, width, height):
    ...
    for i, (text, dur) in enumerate(zip(scripts, durations)):
        ...
        img_array = create_text_frame(width=width, height=height)
        temp_img = os.path.join(temp_dir, f"text_{i}.png")
        Image.fromarray(img_array).save(temp_img)

        # Wrap text để chèn cho drawtext
        font_size = 30
        line_spacing = 10
        ...
        wrapped_text = wrap_text(text, width + 460, font_size=20)
        wrapped_text = wrapped_text.replace(":", "\\:").replace("'", "\\'")
        text_height = calculate_text_height(wrapped_text, font_size=20, line_spacing=10)
        y_position = height - text_height - 30

        # Tính y căn giữa chữ
        # total_text_height = calculate_text_height(text, font, line_spacing)
        ...
        vf = (
            ...
            f"x=(w-text_w)/2:y={y_position}:box=1:boxcolor=black@0.5:boxborderw=10:line_spacing={line_spacing}"
        )

        out_path = os.path.join(temp_dir, f"vid_text_{i}.mp4")
        (
            ffmpeg.input(temp_img, loop=1, t=dur, framerate=fps)
            .filter_("fps", fps=fps)
            .filter_("format", "rgba")
            .drawtext(
                text=wrapped_text,
            ...
            .output(out_path, vcodec='libx264', pix_fmt='yuv420p', t=dur, r=fps, crf=18)
            .overwrite_output()
            .run()
        )
...
text_to_video_tab = gr.Interface(
    ...
    inputs=[
        ...
        gr.Number(label="Width", value=600),
        gr.Number(label="Height", value=800),
    ],
    outputs=[
